...
#    i2c = None
    ...
    def read(self):
            ...
            hibyte = sensor_data[self.__PX4FLOW_PIXEL_FLOW_X + 1]
            if (hibyte > 127):
               hibyte -= 256
            x_velocity = ((hibyte << 8) + sensor_data[self.__PX4FLOW_PIXEL_FLOW_X]) 

            hibyte = sensor_data[self.__PX4FLOW_PIXEL_FLOW_Y + 1]
            if (hibyte > 127):
               hibyte -= 256
            y_velocity = ((hibyte << 8) + sensor_data[self.__PX4FLOW_PIXEL_FLOW_Y]) 
            ...
            if (hibyte > 127):
               hibyte -= 256
            ground_distance = ((hibyte << 8) + sensor_data[self.__PX4FLOW_GROUND_DISTANCE]) / 1000
